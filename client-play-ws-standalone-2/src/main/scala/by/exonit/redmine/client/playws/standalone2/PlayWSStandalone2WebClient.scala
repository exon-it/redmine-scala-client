...
package by.exonit.redmine.client.playws.standalone2
...
import Implicits._
import by.exonit.redmine.client.playws.standalone2.PlayWSStandalone2WebClient.{RequestInterpreter, RequestState, ResponseInterpreter, StreamingResponseInterpreter}

class PlayWSStandalone2WebClient(val client: StandaloneWSClient)
  (implicit mat: Materializer)
  extends WebClient {

  def compileRequestCommand(requestCommand: RequestDSL.Request[Unit]): IO[StandaloneWSRequest] = IO {
  ...
  def compileResponseCommand[T](responseCommand: ResponseDSL.Response[T]): StandaloneWSResponse => IO[T] = response => IO {
  ...
  ): StandaloneWSResponse => IO[T] = response => IO {
...
object PlayWSStandalone2WebClient {
  ...
  class ResponseInterpreter(response: StandaloneWSResponse) extends (ResponseDSL.ResponseOp ~> Id) {
  ...
  class StreamingResponseInterpreter(response: StandaloneWSResponse)(implicit mat: Materializer)
